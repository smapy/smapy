...
2. The tests that cover a module called ``smapy/path/to/a_module.py``
   should be implemented in a separated module called
   ``tests/smapy/path/to/test_a_module.py``.
...
To run a subset of tests::
    ...
    $ python -m pytest tests.test_smapy
...
2. Update the version in ``setup.cfg``, ``smapy/__init__.py`` and
   ``HISTORY.md`` files.
3. Create a new git tag pointing at the corresponding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``smapy/__init__.py``
...
.. _GitHub issues page: https://github.com/csala/smapy/issues
.. _Travis Build Status page: https://travis-ci.org/csala/smapy/pull_requests
